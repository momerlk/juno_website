...
import { junoAdminTheme } from '../admin/juno-admin';
...
const SellerDashboardInner: React.FC = () => {
  ...
  const pageMeta = useMemo(() => {
    ...
    return sortedNav.find((item) => pathname.endsWith(item.href) || pathname.includes(`${item.href}/`))
      ?? { name: 'Dashboard', subtitle: 'Your brand workspace.' };
  }, [pathname]);
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <div className="hidden border-b border-white/5 bg-black/10 px-6 py-5 backdrop-blur-xl md:block">
          <div className="flex items-end justify-between gap-6">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.28em] text-white/35">Juno Studio</p>
...
const SellerDashboard: React.FC = () => (
  <SellerQueueProvider><SellerDashboardInner /></SellerQueueProvider>
);
