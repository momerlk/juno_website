...
import { Product, Variant, Option, Pricing, SizingGuide } from '../../constants/types';
import * as api from '../../api/sellerApi';
import { useSellerAuth } from '../../contexts/SellerAuthContext';
import { X, Plus, Trash2, Upload, DollarSign, Type, Tag, Image as ImageIcon, Paperclip, Settings2, Ruler, AlertTriangle } from 'lucide-react';
...
const ProductEditor: React.FC<ProductEditorProps> = ({ product, onClose }) => {
    ...
    }, [formData.options, generateVariantCombinations, formData.pricing?.price]);
    ...
    const handleSizingGuideChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
        setFormData(prev => ({ ...prev, sizing_guide: { ...prev.sizing_guide, size_fit: e.target.value } as SizingGuide }));
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!hasGenderTag) {
            issues.push("Gender tag (male, female, or unisex) is missing.");
        }
        const isApparel = apparelTypes.includes(formData.product_type || '');
        if (isApparel && !formData.sizing_guide?.size_fit?.trim()) {
            issues.push("Sizing guide is required for this product type.");
        }
...
- ${issues.join("\n- ")}`);
        ...
        setIsSaving(true);
        try {
            const apiCall = product && product.id
                ? api.Seller.UpdateProduct(seller.token, { ...formData, id: product.id } as Product)
                : api.Seller.CreateProduct(seller.token, formData as Product);
    ...
    return (
        <div className="fixed inset-0 bg-black/70 z-50 flex justify-center items-center p-4">
            <motion.div 
                ...
                exit={{ y: 50, opacity: 0 }}
                className="bg-background-light rounded-lg shadow-xl w-full max-w-4xl max-h-[95vh] flex flex-col"
            >
                ...
                <form onSubmit={handleSubmit} className="flex-grow overflow-y-auto p-6 space-y-6">
                    <Section title="Basic Information" icon={<Paperclip className="text-primary" />}>
                        ...
                        <div>
                            ...
                            {!formData.description && <p className='text-xs text-yellow-500 flex items-center mt-1'><AlertTriangle size={14} className='mr-1'/> Description is recommended.</p>}
                        </div>
                        <div className="grid grid-cols-2 gap-6">
                            <div>
                                <label htmlFor="product_type" className="block text-sm font-medium text-neutral-300">Product Type</label>
                                <select name="product_type" id="product_type" value={formData.product_type || ''} onChange={handleChange} className="w-full bg-background border border-neutral-700 rounded-md p-2 text-white mt-1">
                            ...
                            <div>
                                ...
                                <select id="gender" value={currentGender} onChange={handleGenderChange} className="w-full bg-background border border-neutral-700 rounded-md p-2 text-white mt-1">
                                    <option value="">Select gender</option>
                                    <option value="male">Male</option>
                                    <option value="female">Female</option>
                                    <option value="unisex">Unisex</option>
                                </select>
                                {!currentGender && <p className='text-xs text-yellow-500 flex items-center mt-1'><AlertTriangle size={14} className='mr-1'/> Gender tag is recommended.</p>}
                            </div>
                        </div>
                    </Section>
                    ...
                    {isApparel && (
                        <Section title="Sizing Guide" icon={<Ruler className="text-teal-500" />}>
                            <div>
                                ...
                                <textarea id="size_fit" value={formData.sizing_guide?.size_fit || ''} onChange={handleSizingGuideChange} className="w-full bg-background border border-neutral-700 rounded-md p-2 text-white mt-1 h-20" placeholder="e.g., Regular fit, true to size. Model is 6ft and wears a size M."/>
                                {!formData.sizing_guide?.size_fit && <p className='text-xs text-yellow-500 flex items-center mt-1'><AlertTriangle size={14} className='mr-1'/> Sizing guide is required for apparel.</p>}
                            </div>
                        </Section>
                    )}
                    ...
                    <Section title="Media" icon={<ImageIcon className="text-blue-500" />}>
                        <div className="mt-2 grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-4">
                            ...
                            <label htmlFor="image-upload" className="w-full h-24 border-2 border-dashed border-neutral-600 rounded-md flex flex-col justify-center items-center cursor-pointer hover:border-primary text-neutral-400">
                                <Upload size={24} />
                    ...
                    <Section title="Options & Variants" icon={<Settings2 className="text-purple-500" />}>
                        ...
                        {formData.variants && formData.variants.length > 0 && (
                            <div className="mt-4 pt-4 border-t border-neutral-700">
                                <h4 className="font-semibold text-neutral-300 mb-2">Variants</h4>
                                    ...
                                    {formData.variants.map(variant => (
                                        <div key={variant.id} className="grid grid-cols-3 gap-4 items-center bg-background p-2 rounded-md">
                                            ...
                                            <div className="col-span-1">
                                                ...
                                                <input type="number" defaultValue={variant.price} onBlur={e => handleVariantChange(variant.id, 'price', e.target.value)} className="w-full bg-neutral-800 text-white rounded-md p-1 text-sm border border-neutral-700" />
                                            </div>
                                            <div className="col-span-1">
                                                <label className="text-xs text-neutral-500">Stock</label>
                                                <input type="number" defaultValue={variant.inventory?.quantity || 0} onBlur={e => handleVariantChange(variant.id, 'quantity', e.target.value)} className="w-full bg-neutral-800 text-white rounded-md p-1 text-sm border border-neutral-700" />
                                            </div>
                                        </div>
                                    ))}
                                </div>
                    ...
                    <Section title="Organization" icon={<Tag className="text-orange-500" />}>
                        <div>
                            ...
                            <input type="text" id="tags" defaultValue={formData.tags?.join(', ')} onBlur={e => handleTagChange(e.target.value.split(',').map(s => s.trim()).filter(Boolean))} className="w-full bg-background border border-neutral-700 rounded-md p-2 text-white mt-1" placeholder="e.g. summer, new-arrival, sale" />
                        </div>
                    </Section>

                    <div className="pt-6 border-t border-neutral-700 flex justify-end space-x-4 flex-shrink-0">
