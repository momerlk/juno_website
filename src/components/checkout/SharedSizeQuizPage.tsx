...
import { SharedSizeQuiz, Sizing } from '../../api/sizingApi';
...
const SharedSizeQuizPage: React.FC = () => {
  ...
  useEffect(() => {
    void (async () => {
      ...
      setQuiz(res.body.quiz); setProductId(res.body.product_id); setLoading(false);
    })();
  }, [token]);
  ...
  const next = async () => {
    ...
    if (productId) void Sizing.recordProgress(productId, question.id, step).catch(() => undefined);
  ...
  const imageURL = illustration?.light_url;
  ...
  return <main className="min-h-screen bg-[#f4f1ea] px-5 py-10 text-[#201f1b] sm:py-16">
    <section className="mx-auto max-w-md">
      ...
      <div className={`mt-7 grid gap-2 ${question.type === 'visual_single_select' ? 'grid-cols-3 sm:grid-cols-6' : ''}`}>
