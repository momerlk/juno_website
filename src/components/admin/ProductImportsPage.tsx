...
const ProductImportsPage: React.FC = () => {
  ...
  const renderMessage = (message: { type: 'success' | 'error' | 'info'; text: string } | null) => message ? (
    <Banner status={message.type} title={message.text} />
  ) : null;
    ...
    <div className="mt-4 space-y-4 text-neutral-100">
      ...
      <Card padding={4}>
        ...
        <div className="grid gap-3 md:grid-cols-[1fr_1.3fr_auto]">
          ...
          <Button label="Import products" size="sm" icon={actionKey === 'shopify' ? <Loader size={14} /> : <Sparkles size={14} />} onClick={() => void handleShopifyImport()} isLoading={actionKey === 'shopify'} />
        </div>
