        ...
        <div className="relative mx-auto max-w-7xl px-4 md:px-6 pt-24">
            ...
            <div className="grid gap-8 xl:grid-cols-[1.05fr_0.95fr] xl:gap-12">
                ...
                <div className="space-y-5">
                    <div className="relative aspect-[3/4] w-full overflow-hidden rounded-2xl bg-[#101011]">
                ...
                <div className="space-y-8">
                    ...
                    <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
                        ...
                        <div className={`h-44 rounded-2xl ${SkeletonPulse}`} />
                    </div>
                    <div className={`h-8 w-full ${SkeletonPulse}`} />
                    <div className={`h-52 rounded-2xl ${SkeletonPulse}`} />
                    <div className={`h-36 rounded-2xl ${SkeletonPulse}`} />
                </div>
            </div>
        </div>
    </div>
...
const CampaignProductPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadProduct = async () => {
      ...
      setError(null);
      try {
        const [resp, relatedResp] = await Promise.all([
            PublicCampaigns.getPublicCampaignProduct(slug, productId),
        ...
        if (resp.ok) {
          ...
          setRelated(
             relatedResp.ok
                ? asArray(relatedResp.body).filter((item) => item.id !== product.id)
                : []
          );
        } else {
  ...
  useEffect(() => {
    // The sticky mobile bar is now permanently visible
                ...
                <div className="relative overflow-hidden bg-[#101011]">
                    ...
                    <div className="absolute left-5 top-5 z-20 flex flex-wrap gap-2">
                        <span className="rounded-full border border-white/12 bg-black/40 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.24em] text-white/80 backdrop-blur-md">
                            {product.product_type || 'Campaign Piece'}
                        </span>
                        {discountPercentage > 0 ? (
                            <span className="rounded-full border border-primary/40 bg-primary/85 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.24em] text-white shadow-[0_0_15px_2px_rgba(200,3,33,0.30)]">
            ...
            <section className="space-y-8">
                <div>
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        {product.is_featured ? (
                            <span className="inline-flex items-center gap-1.5  border border-white/12 bg-white/[0.05] px-3 py-1 text-[10px] font-bold uppercase tracking-[0.2em] text-white/80">
                                <Sparkles size={11} />
                                Featured
                            </span>
                        ) : null}
                    ...
                    <div className="mt-5 flex flex-wrap items-center gap-4">
                        ...
                        {product.shipping_details?.free_shipping ? (
                            <div className="inline-flex items-center gap-2  border border-white/10 bg-white/[0.03] px-3 py-2 text-sm text-white/75">
                ...
                {asArray(product.tags).length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                        {asArray(product.tags).slice(0, 8).map((tag) => (
                            <span
                                key={tag}
                                className=" border border-white/10 bg-white/[0.04] px-3 py-2 text-[10px] font-bold uppercase tracking-[0.18em] text-white/62"
                            >
                                {tag}
                            </span>
                        ))}
                    ...
                    <div className="grid gap-6 lg:grid-cols-[0.72fr_1fr] lg:items-end">
                        ...
                        <div>
                            <div className="mb-4 flex items-end justify-between gap-4">
                                ...
                                {showAddedFeedback ? (
                                    <span className=" border border-green-400/20 bg-green-400/10 px-4 py-2 text-[11px] font-bold uppercase tracking-[0.2em] text-green-300">
                                        Added to bag
                                    </span>
                                ) : null}
                            </div>
