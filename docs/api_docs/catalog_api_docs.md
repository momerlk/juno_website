...
| `marketing_preview_image` | string | Campaign-only compressed share-image URL; storefront images remain unchanged |
...
| `rating_distribution` | object | Denormalized counts for star ratings `1` through `5`, for PDP headers |
...
| image | Preferred public HTTPS `images[]` value with an image MIME type and dimensions; otherwise the Juno app icon |
| canonical URL | `${FRONTEND_URL}/catalog/${product.id}` |
| seller | `seller_name` is emitted as `product:brand` when present |

`og:image:secure_url` always matches `og:image`. When available, the response
also includes `og:image:type`, `og:image:width`, and `og:image:height`, read
once from the source image while the preview is generated. Generated pages are
held in the process-local cache for 10 minutes (maximum 1,000 entries). Product
writes invalidate the matching preview so its next request rebuilds it. Missing,
unpublished, inactive, or unavailable products serve the unchanged normal Juno
site metadata. The public-site proxy must forward
`/catalog/*` to this API route; otherwise the SPA host will return its generic
entry page before this handler runs.

For active `marketing_campaign` products, `media.marketing_preview_image` is a
public compressed JPEG generated by
`scripts/backfills/generate_campaign_preview_images.go`. Share previews prefer
this URL without replacing the storefront `images[]` array. Run the script once
with its default dry-run, then rerun with `--apply` after checking its output.
